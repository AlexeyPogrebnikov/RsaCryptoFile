...
namespace CryptoFile.IO.Tests.Writing
{
	...
	public class FileWriter_Test
	{
		...
		public void WriteNextBlock1()
		{
			...
			{
				BigNumber number = BigNumber.FromBytes(new[] { 5, 2, 7, 2 });
				writer.WriteNextBlock(number);
			}

			TestHelper.CheckFile(fileName, 5, 2, 7, 2);
		}
		...
		public void WriteNextBlock2()
		{
			...
			{
				BigNumber number = BigNumber.FromBytes(new[] { 45, 23, 129, 243, 4, 12, 56, 90 });
				writer.WriteNextBlock(number);
				number = BigNumber.FromBytes(new[] { 5, 2, 7 });
				writer.WriteNextBlock(number);
			}

			TestHelper.CheckFile(fileName, 45, 23, 129, 243, 4, 12, 56, 90, 5, 2, 7);
		}
		...
		public void WriteNextBlock3()
		{
			...
			{
				BigNumber number = BigNumber.FromBytes(new[] { 87, 32, 107, 42, 58 });
				writer.WriteNextBlock(number);
				number = BigNumber.FromBytes(new[] { 31, 178, 241, 255, 43 });
				writer.WriteNextBlock(number);
			}

			TestHelper.CheckFile(fileName, 87, 32, 107, 42, 58, 31, 178, 241, 255, 43);
		}
