...
namespace CryptoFile.Client.Tests.Crypto
{
	...
	public class RsaFileCipher_Test
	{
		...
		{
			BigNumber e = BigNumber.FromBytes(new[] { 7 });
			BigNumber n = BigNumber.FromBytes(new[] { 217, 118, 184, 189, 114, 147, 142, 61 });
			var key = new PublicKey(e, n);
			string sourceFile = Path.Combine(testFolder, "source.txt");
			File.WriteAllBytes(sourceFile, new byte[] { 104, 101, 108, 108, 111 });
			string destinationFile = Path.Combine(testFolder, "destination.rsa");
		...
		{
			BigNumber e = BigNumber.FromBytes(new[] { 17 });
			BigNumber n = BigNumber.FromBytes(new[] { 3, 81, 217, 6, 4, 255, 114, 123, 36 });
			var key = new PublicKey(e, n);
			string sourceFile = Path.Combine(testFolder, "source.txt");
