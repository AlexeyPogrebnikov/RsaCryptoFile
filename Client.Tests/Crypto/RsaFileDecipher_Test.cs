...
namespace CryptoFile.Client.Tests.Crypto
{
	...
	public class RsaFileDecipher_Test
	{
		...
		{
			string fileName = Path.Combine(testFolder, "file.rsa");
		...
		{
			BigNumber d = BigNumber.FromBytes(new[] { 193, 223, 147, 10, 224, 136, 218, 12 });
			BigNumber n = BigNumber.FromBytes(new[] { 19, 241, 93, 252, 193, 101, 145, 14 });
		...
		public void Decipher()
		{
			...
			string sourceFileName = Path.Combine(testFolder, "source.rsa");
			var bytes = new byte[]
			{
				0, // version
				8, 0, // length of block
			...
			BigNumber d = BigNumber.FromBytes(new[] { 25, 197, 81, 224, 24, 141, 202, 2 });
			BigNumber n = BigNumber.FromBytes(new[] { 91, 22, 25, 68, 22, 58, 67, 15 });
		...
		public void Decipher_ProcessWasStopped()
		{
			...
			string sourceFileName = Path.Combine(testFolder, "source.rsa");
			string destinationFileName = Path.Combine(testFolder, "destination.bin");
			BigNumber e = BigNumber.FromBytes(new[] { 1, 1 });
			BigNumber d = BigNumber.FromBytes(new[] { 25, 197, 81, 224, 24, 141, 202, 2 });
			BigNumber n = BigNumber.FromBytes(new[] { 91, 22, 25, 68, 22, 58, 67, 15 });
			var privateKey = new PrivateKey(d, n);
			var publicKey = new PublicKey(e, n);
			var cipher = new RsaFileCipher(fileFactory);
			string initialFileName = Path.Combine(testFolder, "initial.exe");
