...
namespace CryptoFile.Client.Crypto {
	class RsaFileDecipher : RsaCryptoFile, IRsaFileDecipher {
		...
		public void Decipher(PrivateKey key, string sourceFileName, string destinationFileName) {
			...
			try {
				using (var reader = factory.CreateRsaFileReader(sourceFileName)) {
					reader.ReadHeader();
					...
					using (var writer = CreateFileWriter(destinationFileName, reader)) {
			...
			if (Status == ProcessStatus.Stopped)
				return;
			using (var reader = factory.CreateFileReader(destinationFileName, 1)) {
