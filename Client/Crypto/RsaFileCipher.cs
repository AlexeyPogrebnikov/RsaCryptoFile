...
{
	internal class RsaFileCipher : RsaCryptoFile, IRsaFileCipher
	{
		...
		public void Cipher(PublicKey key, string sourceFileName, string destinationFileName)
		{
			...
			try
			{
				...
				using (IFileReader reader = CreateFileReader(sourceFileName, key))
				{
					TotalBlocks = Convert.ToInt32(Math.Ceiling((double) reader.Length / (key.N.Digit - 1)));
					using (IRsaFileWriter writter = factory.CreateRsaFileWriter(destinationFileName, (short) key.N.Digit))
					{
