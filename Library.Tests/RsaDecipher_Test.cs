...
namespace CryptoFile.Library.Tests
{
	...
	public class RsaDecipher_Test
	{
		...
		public void Decipher1()
		{
			...
			// e = 17
			// d = 145
			BigNumber data = BigNumber.FromBytes(new[] { 87 });
			BigNumber n = BigNumber.FromBytes(new[] { 135, 1 });
			BigNumber d = BigNumber.FromBytes(new[] { 145 });
			var key = new PrivateKey(d, n);
			var rsa = new RsaDecipher(key);
			BigNumber result = rsa.Decipher(data);
			// Check Up
		...
		public void Decipher2()
		{
			...
			// e = 3
			// d = 235
			BigNumber data = BigNumber.FromBytes(new[] { 60 });
			BigNumber n = BigNumber.FromBytes(new[] { 135, 1 });
			BigNumber d = BigNumber.FromBytes(new[] { 235 });
			var key = new PrivateKey(d, n);
			var rsa = new RsaDecipher(key);
			BigNumber result = rsa.Decipher(data);
			// Check Up
