...
namespace CryptoFile.Library.Tests.LongArithmetic
{
	...
	public class BigNumber_FullTest
	{
		...
		{
			IList<int> expectedNumbers = expectedNumber.Numbers;
			IList<int> actualNumbers = actualNumber.Numbers;
		...
		{
			string message = divisible + " / " + divisor + "\n";
		...
		public void Division()
		{
			for (var i = 1; i < 1000; ++i)
			{
				...
				{
					BigNumber divisible = BigNumber.FromInt(i);
					BigNumber divisor = BigNumber.FromInt(j);
					BigNumber actual = divisible.Division(divisor);
					BigNumber expected = BigNumber.FromInt(i / j);
