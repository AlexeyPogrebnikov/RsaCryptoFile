...
namespace CryptoFile.Library.Prime {
	public class RabinMillerTest : IPrimeTest {
		...
		public bool CheckPrimality(BigNumber number) {
			...
			var buff = number.Subtraction(one);
			var degree = zero.Clone();
			do {
			...
			foreach (var witness in witnesses) {
				...
				var power = witness.Power(buff, number);
				var flag = false;
				for (var index = zero.Clone(); index.Compare(degree.Subtraction(one)) == CompareResult.Less; index.Increment()) {
		...
		private static bool CheckPseudoprimeNumber(BigNumber witness, BigNumber r, BigNumber number) {
			var power = witness.Power(r, number);
