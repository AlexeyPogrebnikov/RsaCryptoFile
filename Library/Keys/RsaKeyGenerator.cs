...
namespace CryptoFile.Library.Keys
{
	public class RsaKeyGenerator
	{
		...
		{
			BigNumber euler = Euler(p, q);
			BigNumber e = CalcE(euler, startE);
			BigNumber d = CalcD(euler, e);
			BigNumber n = p.Multiplication(q);
		...
		{
			BigNumber first = p.Subtraction(one);
			BigNumber second = q.Subtraction(one);
			return first.Multiplication(second);
		}

		private static bool CheckPrime(BigNumber number)
		{
			BigNumber last = number.Division(two);
			for (BigNumber i = two.Clone(); i.Compare(last) == CompareResult.Less; i.Increment())
			{
